...
- `shared/` (`@pc-monitor/shared`): zod schemas + inferred types for metrics, processes, actions, logs, config and the Socket.IO event contract (`ws/messages.ts`: `ServerToClientEvents`, `serverEventSchemas`). **Rule: add or change a schema here first, then use it in backend and frontend.** The backend validates requests and generates OpenAPI from these schemas; the frontend derives types from them and validates each Socket.IO event payload with `serverEventSchemas`.
- `backend/src/core/`: env config, Prisma client, `AppError` + global error handler, security middleware, validation middleware, Socket.IO hub (`core/ws`, path `/ws`, websocket transport only), OpenAPI registry. `backend/src/features/{health,metrics,processes,actions,logs,config}`: each with `*.routes.ts`, `*.controller.ts`, `*.service.ts`, `index.ts`. `backend/src/app.ts` builds the Express app (all routes under `/api`), `server.ts` listens and attaches `/ws`. Controllers only handle the happy path and forward errors with `next(new AppError(...))`.
- `frontend/src/{app,core,features/{metrics,processes,actions,logs}}`: each feature owns its `components/`, `hooks/`, `api.ts`.

Data flow:
1. `metrics/ticker.ts` runs every 2s: `snapshot.ts` collects CPU/RAM/disk/network/processes via `systeminformation`, emits a `snapshot` event through the Socket.IO hub, runs `alerts.ts` (writes a `monitor` `Log` and emits `alert` when an `AppConfig` threshold such as `CPU_THRESHOLD` is exceeded for 3 cycles), then persists a sample.
