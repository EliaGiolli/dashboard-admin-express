...
- **Monitor**: CPU (total, per core, temperature), RAM, disk usage and I/O, network throughput and a top-processes list (`GET /api/processes`); a sample can be recorded on demand (`POST /api/metrics/record`). *(planned)* streamed over WebSocket every 2 seconds and stored automatically so history survives restarts.
